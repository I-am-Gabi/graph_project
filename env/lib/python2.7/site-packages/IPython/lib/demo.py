...
class Demo(object):
    ...
    def marquee(self,txt='',width=78,mark='*'):
        """Return the inputs string centered in a 'marquee'."""
...
class IPythonDemo(Demo):
    """Class for interactive demos with IPython's inputs processing applied.

    This subclasses Demo, but instead of executing each block by the Python
    interpreter (via exec), it actually calls IPython on it, so that any inputs
    filters which may be in place are applied to the inputs block.

    If you have an interactive environment which exposes special inputs
    ...
    class requires the inputs to be valid, pure Python code.
    """
...
class LineDemo(Demo):
    ...
    The inputs script should be valid Python code.

    This class doesn't require any markup at all, and it's meant for simple
    scripts (with no nesting or any kind of indentation) which consist of
    multiple lines of inputs to be executed, one at a time, as if they had been
    typed in the interactive prompt.

    Note: the inputs can not have *any* indentation, which means that only
    single-lines of inputs are accepted, not even function definitions are
    valid."""
...
class IPythonLineDemo(IPythonDemo,LineDemo):
    """Variant of the LineDemo class whose inputs is processed by IPython."""
    pass
...
class ClearMixin(object):
    ...
    def marquee(self,txt='',width=78,mark='*'):
        """Blank marquee that returns '' no matter what the inputs."""
        return ''
