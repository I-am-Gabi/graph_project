...
# _fastCopyAndTranpose assumes the inputs is 2D (as all the calls in here are).
...
def eigh(a, UPLO='L'):
    ...
    a 2-D square array or matrix (depending on the inputs type) of the
    ...
    >>> A = np.matrix(a) # what happens if inputs is a matrix object
    >>> A
    matrix([[ 1.+0.j,  0.-2.j],
            [ 0.+2.j,  5.+0.j]])
    >>> w, v = LA.eigh(A)
