...
class CommandLineInterface(object):
    ...
    :param input: :class:`~prompt_toolkit.inputs.Input` instance.
    ...
    def reset(self, reset_current_buffer=False):
        """
        Reset everything, for reading the next inputs.
    ...
    def invalidate(self):
        """
        Thread safe way of sending a repaint trigger to the inputs event loop.
        """
        ...
        def run_async(self, reset_current_buffer=True, pre_run=None):
            ...
            def run():
                ...
                try:
                    ...
                    with self.inputs.raw_mode():
                        self.renderer.request_absolute_cursor_position()
                        self._redraw()

                        yield from self.eventloop.run_as_coroutine(
                                self.inputs, self.create_eventloop_callbacks())
    ...
    def run_sub_application(self, application, done_callback=None, erase_when_done=False,
        ...
        That means, it uses the same inputs and output channels and it shares
        the same event loop.
    ...
    def run_system_command(self, command):
        ...
        def wait_for_enter():
            ...
            This has two advantages over using 'inputs'/'raw_input':
            - This will share the same inputs/output I/O.
        ...
        def run():
            # Try to use the same inputs/output file descriptors as the one,
    ...
    def suspend_to_background(self, suspend_group=True):
        ...
        if hasattr(signal, 'SIGTSTP'):
            def run():
                ...
                # handles the case when inputs is piped from another process.
                if suspend_group:
...
class _StdoutProxy(object):
    ...
    def _write(self, data):
        ...
        Note: print()-statements cause to multiple write calls.
              ...
              text itself is again overwritter by the rendering of the inputs
...
class _SubApplicationEventLoop(EventLoop):
    ...
    loop that is run by the parent application detects inputs, the callbacks
