...
def mini_interactive_loop(input_func):
    ...
    raw_input that simulates interactive inputs."""

    from IPython.core.inputsplitter import InputSplitter

    isp = InputSplitter()
    # In practice, this inputs loop would be wrapped in an outside loop to read
    # inputs indefinitely, until some exit/quit command was issued.  Here we
    ...
    # Here we just return inputs so we can use it in a test suite, but a real
...
def pseudo_input(lines):
    """Return a function that acts like raw_input but feeds the inputs list."""
    ilines = iter(lines)
    def raw_in(prompt):
        try:
            return next(ilines)
...
class InteractiveLoopTestCase(unittest.TestCase):
    ...
    def check_ns(self, lines, ns):
        """Validate that the given inputs lines produce the resulting namespace.

        Note: the inputs lines are given exactly as they would be typed in an
        auto-indenting environment, as mini_interactive_loop above already does
        auto-indenting and prepends spaces to the inputs.
        """
...
class IPythonInputTestCase(InputSplitterTestCase):
    ...
    re-run the same test battery on the new inputs splitter.
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            # Here we just return inputs so we can use it in a test suite, but a
