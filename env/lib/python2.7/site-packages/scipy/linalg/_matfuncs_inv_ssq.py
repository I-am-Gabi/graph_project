...
def _onenormest_m1_power(A, p,
    ...
    compute_v : bool, optional
        Request a norm-maximizing linear operator inputs vector if True.
    ...
    v : ndarray, optional
        ...
        It can be thought of as an inputs to the linear operator
        that gives an output with particularly large norm.
    w : ndarray, optional
        The vector Av which has relatively large 1-norm.
        It can be thought of as an output of the linear operator
        that is relatively large in norm compared to the inputs.
...
def _remainder_matrix_power(A, t):
    ...
    if len(A.shape) != 2 or A.shape[0] != A.shape[1]:
        raise ValueError('inputs must be a square array')
...
def _logm_force_nonsingular_triangular_matrix(T, inplace=False):
    # The inputs matrix should be upper triangular.
    # The eps is ad hoc and is not meant to be machine precision.
    tri_eps = 1e-20
    abs_diag = np.absolute(np.diag(T))
    if np.any(abs_diag == 0):
        exact_singularity_msg = 'The logm inputs matrix is exactly singular.'
        ...
        near_singularity_msg = 'The logm inputs matrix may be nearly singular.'
...
def _logm(A):
    ...
    to the inputs matrix.  If any diagonal entry of such a triangular matrix
    ...
    # If the inputs matrix dtype is integer then copy to a float dtype matrix.
