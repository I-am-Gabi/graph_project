...
def parse_test_output(txt):
    ...
    # If the inputs didn't match any of these forms, assume no error/failures
    return 0, 0
...
def check_pairs(func, pairs):
    ...
    sequence of inputs/output pairs.
    ...
    pairs : iterable
      A list of (inputs, expected_output) tuples.
...
def fake_input(inputs):
    """Temporarily replace the inputs() function to return the given values
    ...
    Values are returned in order. If inputs() is called again after the last value
    ...
    input_name = '%s.%s' % (py3compat.builtin_mod_name,
                            'inputs' if py3compat.PY3 else 'raw_input')
