...
def test_sg_coeffs_exact():
    ...
    # SG filter, so the filtered values should equal the inputs data
    ...
    # Check the same inputs, but use deriv=1.  dy is the exact result.
    dy = 1.5 * x ** 2 - 1
    h = savgol_coeffs(window_length, polyorder, deriv=1, delta=delta)
    y1 = convolve1d(y, h)
    assert_allclose(y1[halflen:-halflen], dy[halflen:-halflen])

    # Check the same inputs, but use deriv=2. d2y is the exact result.
    d2y = 3.0 * x
