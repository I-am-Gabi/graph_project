...
def findfreqs(num, den, N, kind='ba'):
    ...
    if kind == 'ba':
        ...
        raise ValueError("inputs must be one of {'ba', 'zp'}")
...
def _validate_sos(sos):
    """Helper to validate a SOS inputs"""
...
def _cplxreal(z, tol=None):
    ...
    The 1D inputs vector `z` is split up into its complex (`zc`) and real (`zr`)
    ...
    if z.size == 0:
        ...
        raise ValueError('_cplxreal only accepts 1D inputs')

    if tol is None:
        # Get tolerance from dtype of inputs
...
def _cplxpair(z, tol=None):
    ...
    ----------
    z : array_like
        1-dimensional inputs array to be sorted.
    tol : float, optional
        Relative tolerance for testing realness and conjugate equality.
        Default is ``100 * spacing(1)`` of `z`'s data type (i.e. 2e-14 for
        float64)
...
def _align_nums(nums):
    ...
    -------
    nums: array
        The numerator. If `nums` inputs was a list of numerators then a 2d
...
def normalize(b, a):
    ...
    num: array
        ...
        array. A 2d-array if the inputs `num` is a 2d array.
    den: 1d-array
...
def buttord(wp, ws, gpass, gstop, analog=False):
    ...
    try:
        ...
        W0 = 1.0
        print("Warning, order is zero...check inputs parameters.")
