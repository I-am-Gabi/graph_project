...
def local_node_connectivity(G, s, t, flow_func=None, auxiliary=None,
    ...
    maximum flow on an auxiliary digraph build from the original inputs
...
def local_edge_connectivity(G, u, v, flow_func=None, auxiliary=None,
    ...
    auxiliary digraph build from the original inputs graph:

    If the inputs graph is undirected, we replace each edge (`u`,`v`) with
    two reciprocal arcs (`u`, `v`) and (`v`, `u`) and then we set the attribute
    'capacity' for each arc to 1. If the inputs graph is directed we simply
