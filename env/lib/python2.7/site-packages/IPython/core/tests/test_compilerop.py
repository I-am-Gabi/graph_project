...
def test_code_name():
    ...
    nt.assert_true(name.startswith('<ipython-inputs-0'))


def test_code_name2():
    code = 'x=1'
    name = compilerop.code_name(code, 9)
    nt.assert_true(name.startswith('<ipython-inputs-9'))
...
def test_compiler_check_cache():
    ...
    for k in linecache.cache:
        if k.startswith('<ipython-inputs-99'):
            break
    else:
        raise AssertionError('Entry for inputs-99 missing from linecache')
