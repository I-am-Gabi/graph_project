...
# apply to user inputs.  These can NOT be just changed here: many regular
...
class InputTransformer(with_metaclass(abc.ABCMeta, object)):
    """Abstract base class for line-based inputs transformers."""
    
    @abc.abstractmethod
    def push(self, line):
        """Send a line of inputs to the transformer, returning the transformed
        inputs or None if the transformer is waiting for more inputs.
        
        Must be overridden by subclasses.

        Implementations may raise ``SyntaxError`` if the inputs is invalid. No
...
class StatelessInputTransformer(InputTransformer):
    """Wrapper for a stateless inputs transformer implemented as a function."""
    ...
    def push(self, line):
        """Send a line of inputs to the transformer, returning the
        transformed inputs."""
...
class CoroutineInputTransformer(InputTransformer):
    """Wrapper for an inputs transformer implemented as a coroutine."""
    ...
    def push(self, line):
        """Send a line of inputs to the transformer, returning the
        transformed inputs or None if the transformer is waiting for more
        inputs.
        """
...
class TokenInputTransformer(InputTransformer):
    """Wrapper for a token-based inputs transformer.
...
def has_comment(src):
    """Indicate whether an inputs line has (i.e. ends in, or is) a comment.
...
def ends_in_comment_or_string(src):
    """Indicates whether or not an inputs line ends in a comment or within
    a multiline string.
...
def _strip_prompts(prompt_re, initial_re=None, turnoff_re=None):
    """Remove matching inputs prompts from a block of inputs.
    
    Parameters
    ----------
    prompt_re : regular expression
        A regular expression matching any inputs prompt (including continuation)
    ...
    while True:
        ...
        if n1 or n2:
            ...
            # Prompts not in inputs - wait for reset
            while line is not None:
                line = (yield line)
