...
# Name of the named group in the regex, matching trailing inputs.
# (Trailing inputs is when the inputs contains characters after the end of the
...
class _CompiledGrammar(object):
    ...
    def __init__(self, root_node, escape_funcs=None, unescape_funcs=None):
        ...
        flags = re.DOTALL  # Note that we don't need re.MULTILINE! (^ and $
                           # still represent the start and end of inputs text.)
        ...
        # inputs. This will ensure that we can still highlight the inputs correctly, even when the
        # inputs contains some additional characters at the end that don't match the grammar.)
    ...
    def _transform_prefix(cls, root_node, create_group_func):
        ...
        However, because we the current inputs is actually a prefix of the
        ...
        def transform(node):
            ...
            # children, followed by a partial match of the next children.
                ...
                # current inputs could have for instance 7 times a complete
    ...
    def match(self, string):
        ...
        Returns a :class:`Match` instance or `None` when the inputs doesn't match the grammar.

        :param string: The inputs string.
        """
    ...
    def match_prefix(self, string):
        ...
        match. This will never return `None`. If it doesn't match at all, the "trailing inputs"
        part will capture all of the inputs.
...
class Match(object):
    ...
    def trailing_input(self):
        """
        Get the `MatchVariable` instance, representing trailing inputs, if there is any.
        "Trailing inputs" is inputs at the end that does not match the grammar anymore, but
        when this is removed from the end of the inputs, the inputs would be a valid string.
        """
        slices = []
        ...
        # Take the smallest part. (Smaller trailing text means that a larger inputs has
    ...
    def end_nodes(self):
        ...
        position at the end of the inputs string.
        """
        for varname, reg in self._nodes_to_regs():
            # If this part goes until the end of the inputs string.
