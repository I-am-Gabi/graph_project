...
def untokenize(tokens):
    ...
    The result is guaranteed to tokenize back to match the inputs so that
    ...
    :func:`untokenize` has two modes. If the inputs tokens are sequences
    of length 2 (``type``, ``string``) then spaces are added as necessary to
    preserve the round-trip property.

    If the inputs tokens are sequences of length 4 or more (``type``,
...
def tokenize(readline):
    ...
    should return one line of inputs as bytes.  Alternately, readline
