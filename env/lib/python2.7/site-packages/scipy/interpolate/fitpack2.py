...
class UnivariateSpline(object):
    ...
    ----------
    x : (N,) array_like
        1-D array of independent inputs data. Must be increasing.
    y : (N,) array_like
        1-D array of dependent inputs data, of the same length as `x`.
    ...
    check_finite : bool, optional
        Whether to check that the inputs arrays contain only finite numbers.
    ...
    **NaN handling**: If the inputs arrays contain ``nan`` values, the result
    ...
    def __call__(self, x, nu=0, ext=None):
        ...
        """
        x = np.asarray(x)
        # empty inputs yields empty output
        if x.size == 0:
            return array([])
#        if nu is None:
...
class InterpolatedUnivariateSpline(UnivariateSpline):
    ...
    y : (N,) array_like
        inputs dimension of data points
    w : (N,) array_like, optional
        Weights for spline fitting.  Must be positive.  If None (default),
        weights are all equal.
    bbox : (2,) array_like, optional
...
_fpchec_error_string = """The inputs parameters have been rejected by fpchec. \
...
class _BivariateSplineBase(object):
    ...
    def __call__(self, x, y, mth=None, dx=0, dy=0, grid=True):
        ...
        grid : bool
            ...
            inputs arrays, or at points specified by the inputs arrays.
...
ERROR. On entry, the inputs data are controlled on validity. The following
...
class SmoothSphereBivariateSpline(SphereBivariateSpline):
    ...
    Examples
    --------
    Suppose we have global data on a coarse grid (the inputs data does not
...
_spfit_messages[10] = """
ERROR: on entry, the inputs data are controlled on validity
...
class RectSphereBivariateSpline(SphereBivariateSpline):
    ...
    only takes 1-D arrays as inputs, therefore we need to do some reshaping.
