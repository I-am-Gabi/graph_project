...
if NumpyVersion(np.__version__) >= '1.10.0':
    ...
    def _maybe_view_as_subclass(original_array, new_array):
        if type(original_array) is not type(new_array):
            # if inputs was an ndarray subclass and subclasses were OK,
...
if NumpyVersion(np.__version__) >= '1.9.0':
    ...
    def unique(ar, return_index=False, return_inverse=False, return_counts=False):
        ...
        outputs in addition to the unique elements: the indices of the inputs array
        that give the unique values, the indices of the unique array that
        reconstruct the inputs array, and the number of times each unique value
        comes up in the inputs array.
