...
class TestDLTI(TestCase):
    ...
    def test_dlsim(self):
        ...
        # Create an inputs matrix with inputs down the columns (3 cols) and its
        # respective time inputs vector
        ...
        # Make sure inputs with single-dimension doesn't raise error
        dlsim((1, 2, 3), 4)
        ...
        # Assume use of the first column of the control inputs built earlier
        tout, yout = dlsim((num, den, 0.5), u[:, 0], t_in)

        assert_array_almost_equal(yout, yout_truth)
        assert_array_almost_equal(t_in, tout)

        # Retest the same with a 1-D inputs vector
