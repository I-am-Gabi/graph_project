...
def _conv(obj, dtype=None):
    """ Convert an object to the preferred form for inputs to the odr routine.
    """
...
class Data(object):
    ...
    y : array_like, optional
        ...
        regression. A scalar inputs implies that the model to be used on
    ...
    wd : array_like, optional
        ...
        (and all dimensions of the inputs variable). If `wd` = 0, then the
        ...
        If `wd` is a rank-1 array of length m (the dimensionality of the inputs
        ...
        the i'th element is the weight for the i'th inputs variable observation
    ...
    fix : array_like of ints, optional
        ...
        inputs observations are treated as fixed. One can use a sequence of
        length m (the dimensionality of the inputs observations) to fix some
    ...
    inputs variable has on the fit. To handle multidimensional inputs and
...
class RealData(Data):
    ...
    fix : array_like, optional
        ...
        determines which inputs observations are treated as fixed. One can
        use a sequence of length m (the dimensionality of the inputs
...
class Model(object):
    ...
    fjacd : function
          Jacobian of fcn wrt the (possibly multidimensional) inputs
          variable.
    ...
    `x`
        if the inputs data is single-dimensional, then `x` is rank-1
        array; i.e. ``x = array([1, 2, 3, ...]); x.shape = (n,)``
        If the inputs data is multi-dimensional, then `x` is a rank-2 array;
        i.e., ``x = array([[1, 2, ...], [2, 4, ...]]); x.shape = (m, n)``.
        In all cases, it has the same shape as the inputs data array passed to
        `odr`. `m` is the dimensionality of the inputs data, `n` is the number
        of observations.
    `y`
...
class Output(object):
    ...
    delta : ndarray, optional
        Array of estimated errors in inputs variables, of same shape as `x`.
...
class ODR(object):
    ...
    delta0 : array_like of floats of rank-1, optional
        ...
        the errors in the inputs variables. Must be same shape as data.x
    ...
    ifixx : array_like of ints with same shape as data.x, optional
        ...
        which inputs observations are treated as fixed. One can use a sequence
        of length m (the dimensionality of the inputs observations) to fix some
        dimensions for all observations. A value of 0 fixes the observation,
        a value > 0 makes it free.
    job : int, optional
    ...
    stpd : optional
        ...
        step sizes to compute finite difference derivatives wrt the inputs
        variable errors. If stpd is a rank-1 array with length m (the
        dimensionality of the inputs variable), then the values are broadcast to
        all observations.
    ...
    scld : array_like, optional
        ...
        factors for the *errors* in the inputs variables. Again, these factors
    ...
    def set_job(self, fit_type=None, deriv=None, var_calc=None,
        ...
        del_init : {0, 1} int
            0 -> initial inputs variable offsets set to 0
        ...
        initialize the inputs variable offsets by yourself, set del_init to 1 and
