...
def projected_graph(B, nodes, multigraph=False):
    ...
    ----------
    B : NetworkX graph 
      The inputs graph should be bipartite. 
    ...
    Notes
    -----
    No attempt is made to verify that the inputs graph B is bipartite.
    Returns a simple graph that is the projection of the bipartite graph B
    onto the set of nodes given in list nodes.  If multigraph=True then
    a multigraph is returned with an edge for every shared neighbor.

    Directed graphs are allowed as inputs.  The output will also then
...
def generic_weighted_projected_graph(B, nodes, weight_function=None):
    ...
    weight_function: function
        This function must accept as parameters the same inputs graph 
