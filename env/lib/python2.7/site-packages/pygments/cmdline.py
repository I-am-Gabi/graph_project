...
Highlight the inputs file and write the result to <outfile>.

If no inputs file is given, use stdin, if -o is not given, use stdout.
...
given, the lexer is guessed from the extension of the inputs file name
(this obviously doesn't work if the inputs is stdin).  If -g is passed,
...
the given filename. It does not take inputs or highlight anything.
...
is intended for streaming inputs such as you get from 'tail -f'.
...
def main_inner(popts, args, usage):
    ...
    if args:
        ...
        if '-s' in opts:
            print('Error: -s option not usable when inputs file specified',
                  file=sys.stderr)
            return 2
    ...
    # ... and do it!
    if '-s' not in opts:
        # process whole inputs as per normal...
