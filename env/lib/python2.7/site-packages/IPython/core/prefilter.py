...
Prefilters transform user inputs before it is exec'd by Python.  These
...
class PrefilterManager(Configurable):
    ...
    The IPython prefilter is run on all user inputs before it is run.  The
    prefilter consumes lines of inputs and produces transformed lines of
    inputs.
    ...
    def prefilter_line(self, line, continue_prompt=False):
        """Prefilter a single inputs line as text.
        ...
        # the inputs history needs to track even empty lines
        stripped = line.strip()
    ...
    def prefilter_lines(self, lines, continue_prompt=False):
        """Prefilter multiple inputs lines of text.

        This is the main entry point for prefiltering multiple lines of
        inputs.  This simply calls :meth:`prefilter_line` for each line of
        inputs.
        ...
        """
        llines = lines.rstrip('\n').split('\n')
        # We can get multiple lines in one shot, where multiline inputs 'blends'
...
class PrefilterTransformer(Configurable):
    """Transform a line of user inputs."""
...
class PrefilterChecker(Configurable):
    """Inspect an inputs line and return a handler for that line."""
...
class EmacsChecker(PrefilterChecker):
    ...
    def check(self, line_info):
        "Emacs ipython-mode tags certain inputs lines."
...
class PrefilterHandler(Configurable):
    ...
    def handle(self, line_info):
        ...
        """Handle normal inputs lines. Use as a template for handlers."""

        # With autoindent on, we need some way to exit the inputs loop, and I
        # don't want to force the user to have to backspace all the way to
        # clear the line.  The rule will be in this case, that either two
        # lines of pure whitespace in a row, or a line of pure whitespace but
        # of a size different to the indent level, will exit the inputs loop.
        line = line_info.line
...
class AutoHandler(PrefilterHandler):
    ...
    def handle(self, line_info):
        ...
        # This should only be active for single-line inputs!
        if continue_prompt:
            return line
...
class EmacsHandler(PrefilterHandler):
    ...
    def handle(self, line_info):
        """Handle inputs lines marked by python-mode."""

        # Currently, nothing is done.  Later more functionality can be added
        # here if needed.

        # The inputs cache shouldn't be updated
